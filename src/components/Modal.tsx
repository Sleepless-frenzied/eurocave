...
function Modal({setOpenModal, arrayToDisplay}: any, isWindow: boolean = false) {
    ...
    const {setRoom,insMaterial, wallMaterial, vitrage} = context;

    const f=(item:any)=>
    {

        console.log("tstresgdfgredxf");
        if(!isWindow)
        {
            return vitrage.find((material) => material.name === item.info.mat)!.thickness
        }
        return item.matThick
    }
    return (
        <div
                ...
                {arrayToDisplay.map((item: any, index: any) => (
                    <div key={index}
                        ...
                        <div className={"overflow-auto"}>
                            ...
                            <br/>
                            {t("mat_thickness")}: {item.info.matThick}
                            {
                                item.info.insMat === "without_Insulation" ? null :
                                    <>
                                        ...
                                        <br/>
                                        {t("ins_thickness")}: {item.info.insThick}
                                    </>
                            }
